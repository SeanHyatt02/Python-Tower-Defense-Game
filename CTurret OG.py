...
class Turret(pg.sprite.Sprite):
    ...
    def pick_target(self, enemy_group):
        ...
        if closest_enemy:
            self.target = closest_enemy  # Assign the closest target
            self.archer_action = "Attack"  # Set archer action to "attack"
    ...
    # Load new sprite sheet based on direction and action
            if self.relative_direction in ["Left"]:
                archer_path = os.path.join("Assets", "TowerAssets", "UnitsSheets", "Level1", f"Side{self.archer_action}.png")
            elif self.relative_direction == "Right":
                archer_path = os.path.join("Assets", "TowerAssets", "UnitsSheets", "Level1", f"Side{self.archer_action}.png")
                sprite_sheet = pg.image.load(archer_path).convert_alpha()
            ...
            try:
                # Update the archer sprite sheets and animation list
                sprite_sheet = pg.image.load(archer_path).convert_alpha()
    ...
    def upgrade_turret(self):
        if self.upgrade_level < len(TURRET_DATA):
            ...
            self.is_upgrading = True  # Set the upgrading flag to True
            self.is_upgrade_complete = False  # Reset the upgrade completion flag

            # Update archer placement based on upgrade
    ...
    def update_animation_archer(self):
        if not self.is_upgrading:  # Only update if not upgrading
            self.archer_update_time = pg.time.get_ticks()
            animation_speed = 200  # Animation speed
            if self.archer_update_time - self.archer_last_animation_update > animation_speed:
                self.archer_frame_index = (self.archer_frame_index + 1) % len(self.archer_animation_list)  # Cycle frames
                self.archer_image = self.archer_animation_list[self.archer_frame_index]  # Update image
                self.archer_last_animation_update = self.archer_update_time
    ...
    def draw(self, surface):
        ...
        self.rect = self.archer_image.get_rect()
        self.rect.center = (self.archer_x, self.archer_y)
        surface.blit(self.archer_image, self.rect)
